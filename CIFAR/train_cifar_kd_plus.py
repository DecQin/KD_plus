...
from utils_plus import SimKDConfig, SimKDLoss 
...
def train(model, teacher, T_EMB, train_dataloader, optimizer, criterion, args, epoch):
    ...
    for step, (images, labels) in enumerate(train_dataloader):
        ...
        s2 = ' - {:.2f}ms/step - ce_loss: {:.3f} - kd_loss: {:.3f} - nd_loss: {:.3f} - train_loss: {:.3f} - train_acc: {:.3f}'.format(
             1000 * (time.time() - start), loss_dict.get('loss_sup').item(), loss_dict.get('loss_kd').item(), loss_dict.get('loss_nd').item(), train_loss.val, 1-train_error.val)
    ...
    return sup, kd, nd, train_loss.avg, train_error.avg
...
def epoch_loop(model, teacher, train_set, test_set, args):
    ...
    cfg = SimKDConfig()
    criterion = SimKDLoss(cfg).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        sup_loss, kd_loss, nd_loss, train_epoch_loss, train_error = train(model=model,
                                                                                 teacher=teacher,
                                                                                 T_EMB=T_EMB,
        ...
        summary_writer.add_scalar('nd_loss', nd_loss, epoch)
