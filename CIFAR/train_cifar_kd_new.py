...
from utils_new import SANDKDConfig, SANDKDLoss 
...
def train(model, teacher, T_EMB, train_dataloader, optimizer, criterion, args, epoch):
    train_loss = AverageMeter()
    train_error = AverageMeter()

    # 如果 SANDKDLoss 需要 step / total_steps 做软启动
    ...
    for step, (images, labels) in enumerate(train_dataloader):
        step_in_epoch += 1
        global_step = epoch * step_per_epoch + step_in_epoch  # 计算当前走了多少步
        start = time.time()
        ...
        s2 = ' - {:.2f}ms/step - sup_loss: {:.3f} - kd_loss: {:.3f} - geom_loss: {:.3f} - nbr_loss: {:.3f} - train_loss: {:.3f} - train_acc: {:.3f}'.format(
             1000 * (time.time() - start), loss_dict.get('loss_sup').item(), loss_dict.get('loss_kd').item(), loss_dict.get('loss_geom').item(), loss_dict.get('loss_nbr').item(), train_loss.val, 1-train_error.val)
    ...
    geom = loss_dict.get('loss_geom', torch.tensor(0.)).item()
    nbr = loss_dict.get('loss_nbr', torch.tensor(0.)).item()

    # 如果想把 log λ 也写进 tensorboard
    # lgeom = loss_dict.get('lam_geom', torch.tensor(0.)).item()
    # lnbr  = loss_dict.get('lam_nbr',  torch.tensor(0.)).item()

    return sup, kd, geom, nbr, train_loss.avg, train_error.avg
...
def epoch_loop(model, teacher, train_set, test_set, args):
    ...
    cfg = SANDKDConfig()
    criterion = SANDKDLoss(cfg).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        sup_loss, kd_loss, geom_loss, nbr_loss, train_epoch_loss, train_error = train(model=model,
                                                                                 teacher=teacher,
                                                                                 T_EMB=T_EMB,
        ...
        summary_writer.add_scalar('sup_loss', sup_loss, epoch)
        summary_writer.add_scalar('kd_loss', kd_loss, epoch)
        summary_writer.add_scalar('geom_loss', geom_loss, epoch)
        summary_writer.add_scalar('nbr_loss', geom_loss, epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--warm_up", type=float, default=30.0, help='loss weight warm up epochs')
