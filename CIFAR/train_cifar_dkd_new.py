...
def train(model, teacher, T_EMB, train_dataloader, optimizer, criterion, args, epoch):
    ...
    for step, (images, labels) in enumerate(train_dataloader):
        ...
        loss_dict = criterion(
                    ...
                    feat_s=s_emb,
                    feat_t=t_emb,
                    step=current_step + step, total_steps=total_steps
        )
        ...
        s2 = ' - {:.2f}ms/step - sup_loss: {:.3f} - kd_loss: {:.3f} - geom_loss: {:.3f} - train_loss: {:.3f} - train_acc: {:.3f}'.format(
             1000 * (time.time() - start), loss_dict.get('loss_sup').item(), loss_dict.get('loss_kd').item(), loss_dict.get('loss_geom').item(), train_loss.val, 1-train_error.val)
    ...
    return sup, kd, geom, train_loss.avg, train_error.avg
...
def epoch_loop(model, teacher, train_set, test_set, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        sup_loss, kd_loss, geom_loss, train_epoch_loss, train_error = train(model=model,
                                                                                 teacher=teacher,
                                                                                 T_EMB=T_EMB,
...
if __name__ == "__main__":
    ...
    # class-mean
    with open("./run/cifar100_embedding_fea/resnet56_cifar.json", 'r') as f:
        T_EMB = json.load(f)
    f.close()
