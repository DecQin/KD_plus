...
def hcl(fstudent, fteacher):
    loss_all = 0.0
    # 预先创建 1×1 卷积映射（只需创建一次，可放到类里作为 nn.ModuleList）
    adapt_convs = nn.ModuleList([
        nn.Conv2d(s_ch, t_ch, 1, bias=False).to(fstudent[0].device)
        for s_ch, t_ch in zip([s.shape[1] for s in fstudent],
                              [t.shape[1] for t in fteacher])
    ])
    ...
    for i, (fs, ft) in enumerate(zip(fstudent, fteacher)):
        ...
        # 2. 统一通道数
        fs = adapt_convs[i](fs)   # [B, s_ch, H, W] -> [B, t_ch, H, W]
        # 3. 多尺度 MSE
        n, c, h, w = fs.shape
        loss = F.mse_loss(fs, ft, reduction='mean')
        cnt = 1.0; tot = 1.0
