...
from utils_plus2_new import CKDConfig, CKDLoss, CanonicalProj
...
def train(model, teacher, T_EMB, train_dataloader, optimizer, scheduler, criterion, args, epoch):
...
def epoch_loop(model, teacher, train_set, test_set, args):
    # 初始化 wandb 实验
    wandb.init(
        project="res50-MV2_kd+++_Dc",  # 必选：你的项目名称（可自定义）
    ...
    wandb.finish()  # 关闭 wandb 实验，确保日志完整保存
        ...
        # summary_writer.add_scalar('mmd_loss', nd_loss, epoch)
        # summary_writer.add_scalar('orth_loss', nd_loss, epoch)
...
if __name__ == "__main__":
    ...
    if args.teacher_weights:
        ...
        teacher_ckpt = torch.load(args.teacher_weights, weights_only=False)['model']
